...
# Jordan-Spectral Attention / Mixer replacement controls.
# USE_JSA=1 enables replacement of attention blocks with JordanSpectralMixer.
# JSA_LAST_N_LAYERS <= 0 replaces all layers; otherwise replaces only the deepest N layers.
USE_JSA = int(os.environ.get("USE_JSA", "0"))
JSA_RANK = int(os.environ.get("JSA_RANK", "32"))
JSA_LOCAL_K = int(os.environ.get("JSA_LOCAL_K", "2"))
JSA_LAST_N_LAYERS = int(os.environ.get("JSA_LAST_N_LAYERS", "0"))
...
# Jordan-Spectral Mixer:
# low-rank global spectral token mixing + small causal local shifts.
# It is shape-compatible with attention outputs and can replace CausalSelfAttention.
class JordanSpectralMixer(nn.Module):
    def __init__(self, dim: int, jsa_rank: int, local_k: int, max_seq_len: int = 1024):
        ...
        self.local_weights = (
            mx.zeros((self.local_k, dim), dtype=mx.float32)
            if self.local_k > 0
            else None
        )
        ...
        self.basis = mx.cos(
            (math.pi / float(self.max_seq_len)) * (pos + 0.5) * freq
        )  # [T, R]
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if self.local_k > 0 and seqlen > 1 and self.local_weights is not None:
            max_k = min(self.local_k, seqlen - 1)
            for i in range(max_k):
                k = i + 1
                shifted = mx.pad(x[:, :-k, :], ((0, 0), (k, 0), (0, 0)))
                w = self.local_weights[i].astype(x.dtype)[None, None, :]
                local = local + shifted * w

        basis = self.basis[:seqlen].astype(x.dtype)
        spectral_coeffs = x.transpose(0, 2, 1) @ basis  # [B, D, R]
        gate = nn.sigmoid(self.spectral_gate(pooled)).astype(x.dtype)[:, None, :]  # [B, 1, R]
        gated_coeffs = spectral_coeffs * gate
        global_mix = (gated_coeffs @ basis.T).transpose(0, 2, 1)  # [B, T, D]

        mixed = local + global_mix
        return mixed * self.output_scale.astype(x.dtype)[None, None, :]
...
class Block(nn.Module):
    def __init__(
        ...
        self.use_jsa = use_jsa

        if self.use_jsa:
            self.attn = JordanSpectralMixer(dim, JSA_RANK, JSA_LOCAL_K)
        else:
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        for i in range(num_layers):
            use_jsa = False
            if USE_JSA:
                if JSA_LAST_N_LAYERS <= 0:
                    use_jsa = True
                else:
                    use_jsa = i >= (num_layers - JSA_LAST_N_LAYERS)
        ...
        for b in self.blocks:
            ...
            elif isinstance(b.attn, JordanSpectralMixer):
                b.attn.output_scale = mx.zeros_like(b.attn.output_scale)
...
def load_validation_tokens(pattern: str, seq_len: int, max_seqs: int = 0) -> np.ndarray:
    ...
    all_tokens = []
    total_needed = max_seqs * seq_len + 1 if max_seqs > 0 else None
    ...
    for file in files:
        ...
        if total_needed is not None:
            current = sum(arr.size for arr in all_tokens)
...
def main() -> None:
    ...
    val_tokens = load_validation_tokens(
            args.val_files,
            args.train_seq_len,
            int(os.environ.get("VAL_MAX_SEQS", "0")),
        )
