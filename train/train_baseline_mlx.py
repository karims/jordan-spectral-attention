...
USE_ASSM = int(os.environ.get("USE_ASSM", "0"))
ASSM_RANK = int(os.environ.get("ASSM_RANK", "8"))
ASSM_LOCAL_K = int(os.environ.get("ASSM_LOCAL_K", "4"))
...
# Minimal experimental Adaptive Spectral-Shift Mixer:
# global low-frequency token mixing + local causal shifts + lightweight gating.
class AdaptiveSpectralShiftMixer(nn.Module):
    def __init__(self, dim: int, assm_rank: int, local_k: int):
        super().__init__()
        self.dim = dim
        self.assm_rank = max(int(assm_rank), 1)
        self.local_k = max(int(local_k), 0)
        self.local_weights = mx.zeros((self.local_k, dim), dtype=mx.float32) if self.local_k > 0 else None
        self.spectral_gate = CastedLinear(dim, self.assm_rank)
        self.channel_gate = CastedLinear(dim, dim)
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        pos = mx.arange(seqlen, dtype=x.dtype)[:, None]
        freq = mx.arange(self.assm_rank, dtype=x.dtype)[None, :]
        basis = mx.cos((math.pi / float(max(seqlen, 1))) * (pos + 0.5) * freq)
        spectral_coeffs = x.transpose(0, 2, 1) @ basis
        spectral_coeffs = spectral_coeffs.transpose(0, 2, 1)
        gate = nn.sigmoid(self.spectral_gate(pooled)).astype(x.dtype)[:, :, None]
        gated_coeffs = spectral_coeffs * gate
        global_mix = (gated_coeffs.transpose(0, 2, 1) @ basis.T).transpose(0, 2, 1)

        channel = nn.sigmoid(self.channel_gate(pooled)).astype(x.dtype)[:, None, :]
        mixed = channel * (local + global_mix)
...
class Block(nn.Module):
    def __init__(
        ...
        if USE_ASSM:
            self.assm = AdaptiveSpectralShiftMixer(dim, ASSM_RANK, ASSM_LOCAL_K)
            self.assm_norm = RMSNormNoWeight()
            self.assm_scale = mx.ones((dim,), dtype=mx.float32)
    ...
    def __call__(self, x: mx.array, x0: mx.array) -> mx.array:
        ...
        if USE_ASSM:
            assm_out = self.assm(self.assm_norm(x))
            x = x + self.assm_scale.astype(x.dtype)[None, None, :] * assm_out
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        for b in self.blocks:
            ...
            if hasattr(b, "assm"):
                b.assm.out_proj.weight = mx.zeros_like(b.assm.out_proj.weight)
        self.tok_emb.weight = (
